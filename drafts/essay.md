...
The shape should look familiar. The `op: place` here is the same shape as the `op: react` from the previous section: a typed operation, a target, an actor, a timestamp. No schema negotiation. No database column to add. The commit message is the record.
...
That is the shape of git-native publishing in practice. The next section is about what it cannot do.
